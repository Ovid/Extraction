...
from score_countries import INCOME_GROUP_MAP, REGION_MAP, generate_context_facts
...
class TestGenerateContextFacts:
    def test_first_fact_is_raw_value(self, sample_all_indicator_raw):
        ...
        facts = generate_context_facts("wb_gini", 41.5, 60, "USA", sample_all_indicator_raw["wb_gini"])
        assert len(facts) >= 1
    ...
    def test_peer_comparison_generated_when_divergent(self, sample_all_indicator_raw):
        ...
        facts = generate_context_facts("wb_gini", 28.2, 20, "DNK", sample_all_indicator_raw["wb_gini"])
        assert len(facts) >= 1
    ...
    def test_no_region_comparison_when_within_10_percent(self, sample_all_indicator_raw):
        ...
        facts = generate_context_facts("wb_gini", 27.6, 18, "SWE", sample_all_indicator_raw["wb_gini"])
    ...
    def test_unknown_indicator_returns_empty(self, sample_all_indicator_raw):
        ...
        facts = generate_context_facts("nonexistent_key", 50, 50, "USA", {})
        assert facts == []

    def test_country_not_in_region_map(self, sample_all_indicator_raw):
        """Country not in REGION_MAP still gets raw value fact."""
        facts = generate_context_facts("wb_gini", 30.0, 25, "XXX", sample_all_indicator_raw["wb_gini"])
        assert len(facts) >= 1
    ...
    def test_too_few_peers_no_comparison(self):
        ...
        facts = generate_context_facts("wb_gini", 41.5, 60, "USA", sparse_data)
        assert len(facts) <= 1
